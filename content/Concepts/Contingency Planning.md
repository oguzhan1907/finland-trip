...
Contingency planning is the set of backups that keep the roadtrip workable when weather, ferry timing, fatigue, closures, or booking constraints change the plan. For this vault, contingencies should be attached to the relevant route, ferry, lodging, or logistics page where they matter, rather than kept as vague general advice.
...
- Water-activity fallback: if wind, open water, access rules, gear load, or provider availability makes paddling unsafe or awkward, downgrade to a shore walk, swim, sauna, cabin stay, or shorter sheltered-water outing.
- Festival lodging fallback: the exact house checkout date and access details need confirmation because they affect the post-festival route.
- Festival movement fallback: [[Sources/Solstice Festival Official Details]] says the festival site has no camping and the ski lift stops before the public festival day ends, so late-night return plans need a non-assumed fallback.
- Food and resupply fallback:
- Medical or vehicle issue fallback:

## Route-Specific Contingency Notes

- [[Sources/The Archipelago Trail]] creates the first concrete ferry-linked contingency problem in the vault: if a car-relevant crossing is full, delayed, seasonal, or awkwardly timed, the route may need a short-loop or mainland fallback.
- The Small Archipelago Trail is a useful candidate fallback for the full 250 km route, but it still needs current ferry and lodging checks.
...
- Which days are least flexible?
- Which activities need weather alternates?
- What is the fallback if the main ferry or crossing is unavailable?
- What is the fallback if the full Archipelago Trail is too ferry-constrained for the car?
- What is the fallback if a nature stop prohibits camping, off-trail movement, campfires, dog access, or fishing?
- What is the fallback if a planned paddling day has wind, exposed water, landing restrictions, bird-nesting restrictions, or missing gear?
- What is the fallback if the [[Entities/Ruka]] house checkout is 2026-06-21 rather than 2026-06-22?
- What is the fallback if the first post-festival drive day needs to be much shorter than planned?
